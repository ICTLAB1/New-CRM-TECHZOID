 ...
 *
 * MIRRORED IN netlify/lib/outreachSignature.mjs, which is what actually
 * renders the mail that goes out; this one renders the preview. They are
 * pinned together by a parity test, because a preview that does not match
 * what is sent is worse than no preview.
 */

export interface SignatureBadge {
  /** A data URI or an https URL. Uploaded by the company — never generated. */
  src: string;
  /** Alt text, and what a mail client shows when images are blocked. */
  label: string;
  /** Natural size, so the aspect ratio survives. */
  width?: number;
  height?: number;
}
...
export interface SignatureInput {
  ...
  /** "Enterprise Software Licensing | Microsoft | Adobe | …" — the company's
   *  own description of what it does. Free text on purpose: it changes. */
  credentials?: string;

  badges?: SignatureBadge[];

  /** The line in red at the foot. Legal boilerplate, and the reason it is
   *  configurable rather than hard-coded is that it is legal boilerplate. */
  disclaimer?: string;
}
...
export function siteHref(value: string): string {
  const v = String(value ?? "").trim();
  if (!v) return "";
  return /^https?:\/\//i.test(v) ? v : `https://${v}`;
}

const row = (html: string) => `<tr><td style="padding:0;">${html}</td></tr>`;
...
function office(label: string, address: string): string {
...
export function signatureFrom(
  ...
  const sig = (settings["emailSignature"] ?? {}) as Record<string, unknown>;
  ...
  return {
    ...
    badges: Array.isArray(sig.badges) ? (sig.badges as SignatureBadge[]) : [],
